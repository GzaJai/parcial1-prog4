...
export default function Ingredientes() {
  const { ingredientesQuery, deleteMutation } = useIngredientes();
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex items-center justify-between bg-white dark:bg-slate-900/50 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">Control de composición alimentaria y alérgenos.</p>
        </div>
